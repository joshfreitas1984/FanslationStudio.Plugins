...
/// <summary>
/// In-game editor for TextResizer .yaml resizers, built entirely from the spike-confirmed-safe
/// uGUI component set (Canvas/CanvasScaler/GraphicRaycaster/Image/Text/InputField/EventSystem/
/// StandaloneInputModule) via non-generic AddComponent(Il2CppType.From(...)). See
/// .github/copilot-instructions.md "Confirmed-safe pattern for building real uGUI at runtime
/// under IL2CPP" for the underlying constraints this design works within.
///
/// Deliberately avoids UnityEvent.AddListener (Button.onClick, InputField.onValueChanged, etc.)
/// entirely - subscribing generic interop delegate types has an unconfirmed but plausible crash
/// risk under IL2CPP (see item 6 in copilot-instructions.md re: SceneManager.sceneLoaded), and
/// there's no confirmed-safe precedent for it in this repo. Instead, all interaction is driven
/// by per-frame polling: mouse-click hit-testing against tracked rects for buttons/list
/// items/toggles, and comparing each InputField.text against a last-seen cache for edits - the
/// same style already proven safe for scene-change detection and the UI spike's InputField.
/// InputField focus/typing itself still works via Unity's own EventSystem/GraphicRaycaster
/// plumbing - we never need to subscribe to it directly for that to work.
/// </summary>
public static class TextResizerEditorUi
{
    ...
    /// <summary>Must be called every frame (e.g. from the plugin's tick) while the editor may be open.</summary>
    ...
    private static void BuildRoot()
    {
        ...
        var canvas = _root.AddComponent(Il2CppType.From(typeof(Canvas))).TryCast<Canvas>();
        canvas.renderMode = RenderMode.ScreenSpaceOverlay;
        canvas.sortingOrder = short.MaxValue;

        _root.AddComponent(Il2CppType.From(typeof(CanvasScaler)));
        _root.AddComponent(Il2CppType.From(typeof(GraphicRaycaster)));

        EnsureEventSystemExists();

        var panelGo = new GameObject("Panel");
        panelGo.transform.SetParent(_root.transform, false);
        var panelImage = panelGo.AddComponent(Il2CppType.From(typeof(Image))).TryCast<Image>();
        panelImage.color = new Color(0.08f, 0.08f, 0.08f, 0.95f);
        _panel = panelGo.transform.TryCast<RectTransform>();
        ...
        var dragHandleImage = dragHandleGo.AddComponent(Il2CppType.From(typeof(Image))).TryCast<Image>();
    ...
    {
        var existing = UnityEngine.Object.FindObjectOfType(Il2CppType.From(typeof(EventSystem)));
        if (existing != null)
            return;

        var eventSystemGo = new GameObject("FSTextResizerEventSystem");
        eventSystemGo.AddComponent(Il2CppType.From(typeof(EventSystem)));
        eventSystemGo.AddComponent(Il2CppType.From(typeof(StandaloneInputModule)));
    }
    ...
    private static void RebuildListPanel()
    {
        ...
        containerGo.AddComponent(Il2CppType.From(typeof(RectTransform)));
        containerGo.transform.SetParent(_panel, false);
        _listContainer = containerGo.transform.TryCast<RectTransform>();
        ...
        for (var i = 0; i < pageItems.Count; i++)
        {
            ...
            var image = itemGo.AddComponent(Il2CppType.From(typeof(Image))).TryCast<Image>();
            ...
            var label = labelGo.AddComponent(Il2CppType.From(typeof(Text))).TryCast<Text>();
    ...
    private static void RebuildFormPanel()
    {
        ...
        containerGo.AddComponent(Il2CppType.From(typeof(RectTransform)));
        containerGo.transform.SetParent(_panel, false);
        _formContainer = containerGo.transform.TryCast<RectTransform>();
    ...
    // in a generic List<OptionData> and relies on UnityEvent.onValueChanged - both first-use
    // generic-interop risks under IL2CPP, see copilot-instructions.md items 1/6) - built instead
    // from the same plain Image/Text/click-polling primitives already proven safe elsewhere in
    // this file. Clicking the field toggles a popup list of options (rendered by
    // BuildDropdownOverlay, deferred to the end of RebuildFormPanel so it draws on top of
    // everything else).
    private static float CreateEnumDropdownRow(string key, string label, string value, string[] options, float y, float rowHeight)
    {
        ...
        var image = go.AddComponent(Il2CppType.From(typeof(Image))).TryCast<Image>();
        ...
        var textLabel = labelGo.AddComponent(Il2CppType.From(typeof(Text))).TryCast<Text>();
    ...
    private static void BuildDropdownOverlay(string key, string[] options, float fieldY)
    {
        ...
        var overlayImage = overlayGo.AddComponent(Il2CppType.From(typeof(Image))).TryCast<Image>();
        overlayImage.color = new Color(0.12f, 0.12f, 0.12f, 0.98f);
        var overlayRect = overlayGo.transform.TryCast<RectTransform>();
    ...
    private static Text CreateLabel(RectTransform parent, string name, string text, Vector2 offset, Vector2 size, int fontSize, TextAnchor alignment, Color color, bool anchorBottom = false)
    {
        ...
        var label = go.AddComponent(Il2CppType.From(typeof(Text))).TryCast<Text>();
        label.font = GetBuiltinFont();
    ...
    private static InputField CreateInputField(RectTransform parent, string name, Vector2 topLeftOffset, Vector2 size, string initialText)
    {
        ...
        var inputField = go.AddComponent(Il2CppType.From(typeof(InputField))).TryCast<InputField>();
    ...
    private static Font GetBuiltinFont()
    {
        ...
        var fontObj = Resources.GetBuiltinResource(Il2CppType.From(typeof(Font)), "Arial.ttf");
        _cachedFont = fontObj.TryCast<Font>();
        return _cachedFont;
    }
}
