...
// Single Mono (BepInEx6 Mono) finder/attacher implementing all the host-specific interfaces
// used by Shared services (IBehaviourAttacher, ISpriteElementFinder, IPrefabTextFinder). Mono
// compiles directly against the real UnityEngine assemblies, so these Type-based/generic Unity
// API calls are safe to make directly here - unlike Shared, which is compiled against the
// Mono-style stub assemblies. Consolidated into one class so new hooks only need to be added/
// reused in one place per host instead of duplicated across several finder classes.
